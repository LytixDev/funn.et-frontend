...
describe('Test error box message', async () => {
  test('Clicking minus button on error message hides it', async () => {
    const wrapper = mount(ErrorBox, {
      props: {
        modelValue: 'Error message', 'onUpdate:modelValue': (e: string) => wrapper.setProps({modelValue: e})
      },
      global: {
        plugins: [i18n],
      },
  ...
  test('Error message is not shown if message prop is empty', () => {
    const wrapper = mount(ErrorBox, {
      props: {
        modelValue: '', 'onUpdate:modelValue': (e: string) => wrapper.setProps({modelValue: e})
      },
      global: {
        plugins: [i18n],
      },
